...
(function () {
            ...
            if (quest.rewards && quest.rewards.length > 0) {
                const items = quest.rewards.map(r => `
                    <div class="drop-item rarity-${(r.item?.rarity || 'common').toLowerCase()}">
                        <div class="drop-item-icon rarity-image-wrap" style="padding:2px;border-radius:6px;width:32px;height:32px;flex-shrink:0;">
                            <div class="rarity-curve"></div>
                            ${r.item?.icon ? `<img src="${r.item.icon}" alt="${r.item.name}" class="item-img" style="width:100%;height:100%;object-fit:contain;" />` : `<div style="display:flex;align-items:center;justify-content:center;opacity:0.3;z-index:2;width:100%;height:100%;"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg></div>`}
                        </div>
    ...
    function openLoadoutPicker(slotName) {
        ...
        loadoutPickerGrid.innerHTML = mfItems.map(item => {
            ...
            const iconSvg = getWeaponIconSVG(item.name);
            return `
            <div class="item-card rarity-${r}" data-id="${item.id}" data-name="${item.name.toLowerCase()}" style="cursor:pointer">
                <div class="rarity-image-wrap item-card-image-wrap">
                    <div class="rarity-curve"></div>
                    ${item.icon ? `<img src="${item.icon}" alt="${item.name}" loading="lazy" class="item-img" />` : `<div style="display:flex;align-items:center;justify-content:center;opacity:0.3;width:100%;height:100%">📦</div>`}
                </div>
                <div class="item-card-footer">
                    <span class="item-type-icon">${iconSvg}</span>
                    <span class="item-card-name">${item.name}</span>
    ...
    if (loadoutPickerGrid) {
        loadoutPickerGrid.addEventListener('click', (e) => {
            ...
            if (slotEl) {
                slotEl.innerHTML = `
                    <div class="rarity-${(item.rarity || 'common').toLowerCase()}" style="display:flex;align-items:center;gap:.5rem;padding:.5rem">
                        <div class="rarity-image-wrap" style="padding:4px;border-radius:6px;width:38px;height:38px;flex-shrink:0;">
                            <div class="rarity-curve"></div>
                            ${item.icon ? `<img src="${item.icon}" style="width:100%;height:100%;object-fit:contain;z-index:2;" />` : `<div style="display:flex;align-items:center;justify-content:center;opacity:0.3;z-index:2;width:100%;height:100%;"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg></div>`}
                        </div>
                        <div>
